...
public class PlexController : Controller
{
    ...
    private async Task<string> getLibrary()
    {
    ...
    private async Task<bool> isLibraryType(string LibraryType, int LibraryId)
    {
        var raw_library = await this.getLibrary();
        if ("" == raw_library || null == raw_library)
        {
            return false;
        }
        var json = JsonConvert.DeserializeObject<LibrariesResponse>(raw_library);
        ...
        var d = json.MediaContainer.Directory;
        ...
        IEnumerable<LibraryListing> result = d.Where(lib =>
        {
            return (
    ...
    public async Task<IActionResult> GetLbraries()
    {
        try
        {
            var content = await this.getLibrary();
            if ("" == content)
            {
    ...
    {
        var valid = await this.isLibraryType(LibraryTypes.MOVIE, LibraryId);
    ...
    {
        var valid = await this.isLibraryType(LibraryTypes.SHOW, LibraryId);
